...
locals {
  ...
  iam_role_name                  = coalesce(var.iam_role_name, format("%s-cluster", var.cluster_name))
  iam_role_policy_prefix         = "arn:${data.aws_partition.current.partition}:iam::aws:policy/"
  cluster_encryption_policy_name = coalesce(var.cluster_encryption_policy_name, format("%s-ClusterEncryption", local.iam_role_name))
...
data "aws_iam_policy_document" "assume_role_policy" {
  ...
  statement {
    ...
    principals {
      type        = "Service"
      identifiers = [format("eks.%s", local.dns_suffix)]
    }
    ...
    dynamic "principals" {
      ...
      content {
        type        = "Service"
        identifiers = [format("ec2.%s", local.dns_suffix)]
      }
    }
  }
}
...
resource "aws_iam_role" "this" {
  ...
  name_prefix = var.iam_role_use_name_prefix ? format("%s%s", local.iam_role_name, var.prefix_separator) : null
...
resource "aws_iam_role_policy_attachment" "this" {
  for_each = {
    for key, value in {
      AmazonEKSClusterPolicy         = local.create_outposts_local_cluster ? format("%s/AmazonEKSLocalOutpostClusterPolicy", local.iam_role_policy_prefix) : format("%s/AmazonEKSClusterPolicy", local.iam_role_policy_prefix),
      AmazonEKSVPCResourceController = format("%s/AmazonEKSVPCResourceController", local.iam_role_policy_prefix),
    } : key => value if local.create_iam_role
  }
...
resource "aws_eks_addon" "this" {
  ...
  for_each = {
    for key, value in var.cluster_addons : key => value if !try(value.before_compute, false) && local.create && !local.create_outposts_local_cluster
  }
